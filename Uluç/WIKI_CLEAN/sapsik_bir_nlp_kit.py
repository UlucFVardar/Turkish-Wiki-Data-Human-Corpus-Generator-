...
class sapsik_bir_nlp_kit():
    ...
    def clean_jsonvalues(self,infobox):
        try:
            ...
            for key in infobox.keys():
                ...
                if new_key.decode('utf-8') in self.key_banned or infobox[key].decode('utf-8') == "" or '<!--'.decode('utf-8') in infobox[key].decode('utf-8') or 'yalın liste|'.decode('utf-8') in infobox[key].decode('utf-8'):
                    continue
    ...
    def key_map(self,data):
        ...
        if data.decode('utf-8') in self.key_name_map:
            return 'ad'.decode('utf-8')
        if data.decode('utf-8') in self.key_birth_map:
    ...
    def date_map(self, date_value ):
        ...
        try : 
            ...
            if len(value) == 3:
                converted_date = "%s %s %s"%(value[2], self.month[value[1]-1],value[0])
                ...
                if year1 >year2:
                    ...
                    converted_date = "%s %s %s"%(value[2+3], self.month[value[1+3]-1],value[0+3])
                    return converted_date
            elif len(value) == 1:
